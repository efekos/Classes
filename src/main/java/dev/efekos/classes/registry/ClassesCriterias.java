...
public final class ClassesCriterias {
    ...
    public static final ILevelCriteria TAKE_DAMAGE = register("take_damage",event -> {
        if(event instanceof EntityDamageEvent e)return e.getEntity() instanceof Player;else return false;
    });
    ...
    public static final ILevelCriteria KILL_PLAYER = register("kill_player",event -> {
        if(event instanceof PlayerDeathEvent e) return e.getEntity().getKiller()!=null;return false;
    });

    public static final ILevelCriteria KILL_ENTITY = register("kill_entity",event -> {
        if(event instanceof EntityDeathEvent e) return e.getEntity().getKiller()!=null;return false;
    });
