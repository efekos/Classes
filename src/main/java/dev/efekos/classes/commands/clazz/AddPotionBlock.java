...
@Command(name = "addpotionblock",description = "Add a potion block for a class",permission = "classes.addblock.potion")
public final class AddPotionBlock extends SubCommand {
    @Override
    ...
    public @NotNull Syntax getSyntax() {
        return new Syntax()
                ...
                .withArgument(new PotionNameArgument(ArgumentPriority.REQUIRED,"enchantment"));
    }

    public AddPotionBlock(@NotNull String name) {
        super(name);
    }

    public AddPotionBlock(@NotNull String name, @NotNull String description, @NotNull String usageMessage, @NotNull List<String> aliases) {
    ...
    public void onPlayerUse(Player player, String[] args) {
        ...
        if(clas.getBlockedPotions().contains(PotionEffectType.getByKey(NamespacedKey.fromString(args[1])))){
            player.sendMessage(TranslateManager.translateColors(Main.LANG.getString("commands.add-potion-block.already","&cThat potion is already blocked for that class.")));
            return;
        }
        clas.getBlockedPotions().add(PotionEffectType.getByKey(NamespacedKey.fromString(args[1])));
        Main.CLASSES.update(clas.getUniqueId(),clas);

        player.sendMessage(TranslateManager.translateColors(Main.LANG.getString("commands.add-potion-block.done","&aSuccessfully blocked &b%potion% &afor &b%class%&a!")
